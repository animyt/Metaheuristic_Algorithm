...
namespace TLBO
{
    public partial class Form4 : Form
    {
        ...
        private void UpdateParamView()
        {
            ...
            dataGridView1.Rows[2].Cells[1].Value = ReplaceCount; 
            dataGridView1.Rows[3].Cells[1].Value = ReplaceFrequency; 
            dataGridView1.Rows[4].Cells[1].Value = Teach; 
            dataGridView1.Rows[5].Cells[1].Value = Stud; 
            // Средняя приспособленность
            if (algst.AverF.Count > 0 && t < algst.AverF.Count)
            {
                dataGridView1.Rows[6].Cells[1].Value = Math.Round(algst.AverF[t], 6);
            }
            else
            {
            ...
            if (algst.Population.Count > 0)
            {
                ...
                if (best != null)
                {
                    dataGridView1.Rows[7].Cells[1].Value = "(" + Math.Round(best.x[0], 4).ToString() + "; " +
                    Math.Round(best.x[1], 4).ToString() + ")"; // (x*, y*) Лучшее решение
                    dataGridView1.Rows[8].Cells[1].Value = Math.Round(best.f, 6).ToString(); // f* Значение лучшего решения
                    dataGridView1.Rows[9].Cells[1].Value = exact.ToString(); // Точное решение
                    dataGridView1.Rows[10].Cells[1].Value = Math.Round((double)Math.Abs(exact - best.f), 6).ToString(); // Отклонение от точного решения
                ...
                else
                {
                    ...
                    dataGridView1.Rows[10].Cells[1].Value = "N/A";
        ...
        private void InitDataGridView1()
        {
            ...
            dataGridView1.Rows[1].Cells[0].Value = "Количество учеников:";
            dataGridView1.Rows[2].Cells[0].Value = "Число заменяемых учеников:";
            dataGridView1.Rows[3].Cells[0].Value = "Частота замены учеников:";
            dataGridView1.Rows[4].Cells[0].Value = "Коэффициент учителя:";
            dataGridView1.Rows[5].Cells[0].Value = "Коэффициент учеников:";
            dataGridView1.Rows[6].Cells[0].Value = "Среднее значение f:";            
            dataGridView1.Rows[7].Cells[0].Value = "(x*, y*) Лучшее решение:";
            dataGridView1.Rows[8].Cells[0].Value = "f* Значение лучшего решения:";
            dataGridView1.Rows[9].Cells[0].Value = "Точное решение:";
            dataGridView1.Rows[10].Cells[0].Value = "Отклонение от точного решения:";
        ...
        private void pictureBox3_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (Red[3] == true)
            {
                e.Graphics.DrawLine(p3, 299, 150, 299, 30);
                e.Graphics.DrawLine(p3, 299, 85, 304, 95);
                e.Graphics.DrawLine(p3, 298, 85, 293, 95);

            }
            else
            {
                e.Graphics.DrawLine(p2, 299, 150, 299, 30);
                e.Graphics.DrawLine(p2, 299, 85, 304, 95);
                e.Graphics.DrawLine(p2, 298, 85, 293, 95);
            }



            if ((((Red[9] == true) && ((t - 1) % algst.ReplaceFrequency != 0))||((Red[9] == true)&& ((t - 1) == 0)))|| ((algst.ReplaceCount == 0)&& (Red[9] == true)))
            {
            ...
            if (((Red[9] == true) && ((t - 1) % algst.ReplaceFrequency == 0)&&((t-1)!= 0 ))&& (algst.ReplaceCount > 0))
            {
                e.Graphics.DrawLine(p3, 252, 82, 96, 153);
                e.Graphics.DrawLine(p3, 107, 149, 118, 149);
                e.Graphics.DrawLine(p3, 107, 149, 113, 139);
            }
            else
            {

                e.Graphics.DrawLine(p2, 252, 82, 96, 153);
                e.Graphics.DrawLine(p2, 107, 149, 118, 149);
                e.Graphics.DrawLine(p2, 107, 149, 113, 139);
        ...
        public double stepInd0;
        public double stepInd1;
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            m = 0;
            algst = new Algoritm(); //
            algst.Init(Ip, K, obl, n, z, ReplaceCount, ReplaceFrequency, Teach, Stud);
            ...
            {
                dataGridView1.Rows[6].Cells[1].Value = Math.Round(algst.AverF[t-1], 6); // Средняя приспособленность
            }
            else
            {
                dataGridView1.Rows[6].Cells[1].Value = "N/A";
            ...
            if (algst.Population.Count > 0)
            {
                ...
                if (best != null)
                {
                    dataGridView1.Rows[7].Cells[1].Value = "(" + Math.Round(best.x[0], 4).ToString() + "; " + Math.Round(best.x[1], 4).ToString() + ")"; // (x*, y*) Лучшее решение
                    dataGridView1.Rows[8].Cells[1].Value = Math.Round(best.f, 6).ToString(); // Значение лучшего решения
                    dataGridView1.Rows[9].Cells[1].Value = exact.ToString(); // Точное решение
                    dataGridView1.Rows[10].Cells[1].Value = Math.Round((double)Math.Abs(exact - best.f), 6).ToString(); // Отклонение от точного решения
            ...
            {
                MessageBox.Show("Перейдите к шагу \"Замена учеников\".");
            }
            ...
            //if (flag == true)
            {
                ...
                //if (flag == true)
                {
                    ...
                    for (int i = 0; i < algst.Population.Count; i++)
                    {
                        ...
                        float r = 5;
                        Brush stuColor = Brushes.Blue;
                        int ten = Convert.ToInt32(Math.Round((double)Ip / 10))+1;
                        var top10 = algst.Population.OrderByDescending(s => s.f).Take(ten).ToList();

                        // 2. Создаем HashSet для быстрого поиска
                        var top10Set = new HashSet<object>(top10);
                        if (i == i0)
                        {
                            r = 7;
                            stuColor = Brushes.Red;
                        }
                        else if (top10Set.Contains(algst.Population[i]))
                        {
                            r = 6;
                            stuColor = Brushes.Magenta;
                        }
                        e.Graphics.FillEllipse(stuColor,
                            (float)((stu.x[0] * k - x1) * w / (x2 - x1) - r / 2),
                            (float)(h - (stu.x[1] * k - y1) * h / (y2 - y1) - r / 2), r, r);
                    }
                }
                if (Red[3] == true)
                {
                    var pen1 = new Pen(Color.DarkCyan);

                    // **СОРТИРУЕМ ПОПУЛЯЦИЮ ПЕРЕД ВЫДЕЛЕНИЕМ**
                    var sortedPopulation = algst.Population.OrderByDescending(s => s.f).ToList();

                    // Начинаем цикл с конца отсортированной популяции, выделяя ReplaceCount последних элементов
                    for (int i = sortedPopulation.Count - 1; i >= Math.Max(0, sortedPopulation.Count - ReplaceCount); i--)
                    {
                        var fish = sortedPopulation[i]; // Используем отсортированную популяцию
                        var r2 = 12;
                        float x = (float)((fish.x[0] * k - x1) * w / (x2 - x1) - r2 / 2);
                        float y = (float)(h - (fish.x[1] * k - y1) * h / (y2 - y1) - r2 / 2);
                        e.Graphics.DrawEllipse(pen1, x, y, r2, r2);
                    }
                }
                //if (Red[2] == true)
                //{
                //    for (int i = 0; i < algst.Population.Count; i++)
                //        for (int j = 0; j < algst.Population[i].Kloni.Count; j++)
                //            e.Graphics.FillEllipse(Brushes.DarkCyan, (float)((algst.Population[i].Kloni[j].x[0] * k - x1) * w / (x2 - x1) - 1), (float)(h - (algst.Population[i].Kloni[j].x[1] * k - y1) * h / (y2 - y1) - 1), 2, 2);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Перейдите к шагу \"Фаза учителя\".");
            }
            else if ((Red[3] == true) && (Red[1] == false))
            {
                MessageBox.Show("Перейдите к шагу \"Замена учеников\".");
            }
                // фаза ученика
                if ((t % algst.ReplaceFrequency != 0) || (t == 0) || (algst.ReplaceCount==0))
                {
                if (Red[1] == true)
                {
                    Red[1] = false;
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            else if ((Red[3] == false)&& ((Red[9] == true)|| (Red[0] == true)))
            {
                MessageBox.Show("Перейдите к шагу \"Фаза учителя\".");
            }
            else if ((Red[3] == false) && (Red[1] == true))
            {
                MessageBox.Show("Перейдите к шагу \"Фаза учеников\".");
            }
           /* if (t % algst.ReplaceFrequency != 0)
            {
                MessageBox.Show($"Замена учеников выполняется только каждые {algst.ReplaceFrequency} итераций.\nТекущая итерация: {t}");
                return;
            }*/
            // замена
            if (Red[3] == true)
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            if (Red[7] == true)
            {
                ...
                dataGridView1.Rows[7].Cells[1].Value = "(" + Math.Round(algst.Population[0].x[0], 4).ToString() + "; " + Math.Round(algst.Population[0].x[1], 4).ToString() + ")";
                dataGridView1.Rows[8].Cells[1].Value = Math.Round(algst.Population[0].f, 6).ToString();
                dataGridView1.Rows[9].Cells[1].Value = exact.ToString();
                dataGridView1.Rows[10].Cells[1].Value = Math.Round((double)Math.Abs(exact - algst.Population[0].f), 6).ToString();
        ...
        private void button12_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Получить ответ невозможно! \r\nПерейдите к шагу \"Фаза учеников\".");
            }
            else if (Red[3] == true)
            {
                MessageBox.Show("Получить ответ невозможно! \r\nПерейдите к шагу \"Замена учеников\".");
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if ((Red[0] == true) ||  Red[9] == true)
            {
                ...
                for (int i = t; i < K+1; i++)
                {
                    ...
                    dataGridView1.Rows[7].Cells[1].Value = "(" + Math.Round(algst.Population[i0].x[0], 4).ToString() + "; " + Math.Round(algst.Population[i0].x[1], 4).ToString() + ")";
                    dataGridView1.Rows[8].Cells[1].Value = Math.Round(algst.Population[i0].f, 6).ToString();
                    dataGridView1.Rows[9].Cells[1].Value = exact.ToString();
                    dataGridView1.Rows[10].Cells[1].Value = Math.Round((double)Math.Abs(exact - algst.Population[i0].f), 6).ToString();
